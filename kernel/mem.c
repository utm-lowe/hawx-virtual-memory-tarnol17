...
vm_map_range(pagetable_t pagetable, uint64 va, uint64 size, int perm)
{
    ...
    for(uint64 cur = start; cur < end; cur += PGSIZE) {
      void *p = vm_page_alloc();
      if(p == 0)
        return -1;
      if(vm_page_insert(pagetable, cur, (uint64)p, perm) != 0)
        return -1;
    }
    return 0;
}
...
kernel_map_range(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
  ...
  uint64 curpa = pa;
  for(uint64 cur = start; cur < end; cur += PGSIZE) {
    pte_t *pte = walk_pgtable(pagetable, cur, 1);
    if(pte == 0)
      return -1;
    *pte = PA2PTE(curpa) | perm | PTE_V;
    curpa += PGSIZE;
  }
  return 0;
}
